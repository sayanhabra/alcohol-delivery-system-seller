...
//     // Wire up 401-on-refresh handler → auto logout
...
class AuthNotifier extends AsyncNotifier<AuthState> {
  ...
  Future<AuthState> _tryAutoLogin() async {
    try {
      ...
      // ═══════════════════════════════════════════════════════
      // FIX: Extract nested user from { response: { user: {...} } }
      // ═══════════════════════════════════════════════════════
      ...
      if (responseData is Map) {
        ...
        if (responseObj is Map) {
          userMap = responseObj['user'] as Map<String, dynamic>?;
        }
      }
  ...
  Future<void> updateUserVerificationStatus(SellerStatus status) async {
    ...
    state = AsyncValue.data(
      AuthAuthenticated(
        ...
        user: current.user.copyWith(verificationStatus: status),
      ),
    );
  }
}
